...
public class MercurialHistory extends FileHistory {
	...
	public void refresh(IProgressMonitor monitor, int from) throws CoreException {
		...
		if (!MercurialUtilities.getGpgExecutable().equals("false") && HgClients.getPreference(MercurialPreferenceConstants.PREF_SIGCHECK_IN_HISTORY, "false").equals("true")) { //$NON-NLS-1$ //$NON-NLS-2$
		...
		for (ChangeSet cs : changeSets) {
			...
			revisions.add(new MercurialRevision(cs, gChangeSets.get(Integer.valueOf(cs
					.getChangesetIndex())), resource, sig));
		}
	}
